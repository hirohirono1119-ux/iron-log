...
| 項目 | 決定 | 設計への反映 |
|---|---|---|
| 対象取引所 | **海外も使う** | 海外大手(Binance グローバル等)は**監視専用データソース**(口座不要)。執行は国内登録業者のみ — 保有口座の bitFlyer に加え **Binance Japan** が有力候補(§1.1.1〜1.1.2)。グローバル版海外取引所は日本居住者は取引不可のため執行先にしない |
...
| リーダー予備 | Coinbase / OKX 等 | BTC/USD(T) | 同上(Binance 障害時の比較用、余裕があれば) |
| フォロワー | bitFlyer | BTC/JPY | best bid/ask + 約定 + 板上位数段 |
| フォロワー | Binance Japan | BTC/JPY | 同上(❓口座が Japan 版か要確認) |
...
1. **Binance 口座の種別確認**: 保有している Binance 口座が **Binance Japan**(移行済み)か、旧グローバル口座のままか。Japan 版なら執行先2社目としてそのまま使える。グローバル版のままなら Binance Japan の口座開設が必要
