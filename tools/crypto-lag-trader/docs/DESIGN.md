...
| 執行先口座 | **bitFlyer + Binance Japan(保有)+ GMOコイン + bitbank(初期から開設)の4社** | 候補比較は §1.1.3。Coincheck のみ Phase 2 のエッジ集計を見てから判断 |
| 対象通貨 | **まず BTC、拡張余地を残す** | 通貨ペアは設定ファイルのリストで管理し、全コンポーネントを複数シンボル前提で実装。初期設定は BTC のみ(§4) |
| 言語 | **Python** | Python 3.12 + asyncio で確定(§4) |
...
- 執行先が複数社(§1.1.3 の4社)になると、複数取引所に資金を置いて「安い方で買い・高い方で売り」を同時に行う**両建て型アービトラージ**(方向リスクなし)が国内 JPY 建てだけで組めるようになる。§9 のスコープ外としていたが、現実的な拡張候補に格上げする

### 1.1.3 執行先口座の構成

初期から執行先(watch+trade)とする4社:

| 取引所 | 特徴 | 口座 |
|---|---|---|
| **bitFlyer** | BTC/JPY の国内最大級の流動性。API(Lightning)が充実 | 保有 |
| **Binance Japan** | 手数料が安い。取扱通貨最多で将来の通貨拡張にも有利 | 保有 |
| **GMOコイン** | **maker 手数料がマイナス(リベート)**、taker も低率。WS API の品質に定評。指値中心の戦略なら手数料面で最有力 | 開設する |
| **bitbank** | maker マイナス手数料。アルト系の板が国内では厚め。通貨拡張時に有利 | 開設する |

追加候補(watch のみで開始):

| 取引所 | 特徴 | 判断 |
|---|---|---|
| **Coincheck** | BTC 取引所の手数料が無料。ただし API・板の薄さは要確認 | Phase 2 のエッジ集計を見てから |

監視(データ収集)は公開データなので口座不要。Coincheck も Phase 1 から watch として購読し、Phase 2 のレポートの「取引所ペアごとのエッジ集計」でエッジが見えたら口座開設して trade に昇格する。GMOコイン・bitbank の口座開設は Phase 1 の収集と並行して進めればよい(APIキーが必要になるのは Phase 3 以降)。
...
| 項目 | 採用 | 理由 |
|---|---|---|
| 言語 | **Python 3.12 + asyncio(決定)** | 取引所ライブラリと分析エコシステムが最も充実。個人ツールの開発速度重視 |
...
| watch+trade | GMOコイン(口座開設予定) | BTC/JPY | 同上 |
| watch+trade | bitbank(口座開設予定) | BTC/JPY | 同上 |
| watch | Coincheck | BTC/JPY | 同上(エッジが見えたら口座開設 → trade 昇格) |
...
意思決定はすべて完了(冒頭の決定事項参照)。残るのは Coincheck の trade 昇格判断のみで、これは Phase 2 のエッジ集計を見てから(§1.1.3)。
...
並行タスク(ユーザー側): GMOコイン・bitbank の口座開設。APIキーが実際に必要になるのは Phase 3 以降なので急ぎではない。
...
実装前の最終レビューで洗い出した、見落としがちな論点。

### 8.1 APIキーの権限と保管
- APIキーには**出金権限を付与しない**(取引・照会のみ)。万一キーが漏れても資金流出しない
- IP制限が使える取引所(GMOコイン等)では自宅IPに限定
- キーは `.env` のみに置き、リポジトリは private。`.gitignore` で `.env` と `data/` を除外

### 8.2 税務・損益記録
- 個人の暗号資産損益は雑所得。原価計算は(届出しない場合)**総平均法**がデフォルト
- この戦略は取引回数が多くなるため手計算は不可能。**全約定履歴の保存と年間損益の自動集計出力**を Phase 4(実弾開始)までに必須で用意する
- 取引記録は取引所側の履歴とツール側の記録を突合できる形式(注文ID付き)で残す

### 8.3 取引所の定期メンテナンス
- 国内取引所は週次等の定期メンテナンスで API・板が止まる。メンテ時間帯を `exchanges.yaml` に持たせ、**メンテ前はポジションを持ち越さない**(メンテ開始 N 分前から新規エントリー禁止+強制決済)
- Collector 側はメンテによる欠損を「異常」と区別して記録する(誤アラート防止)

### 8.4 注文ライフサイクル管理
- Execution Engine は注文を状態機械(送信中 → 受付済 → 部分約定 → 全約定/キャンセル/期限切れ)として管理する
- **「注文を出したか分からない」状態**(送信タイムアウト)では、必ず照会で確定させてから次の行動をとる。client order ID を自前で採番し二重発注を防止
- 部分約定で残った端数ポジションも決済対象として追跡する

### 8.5 複数取引所の残高管理
- 執行4社に JPY を分散配置するため、起動時+定期で残高を照会し、Risk Manager が「取引所ごとの発注可能額」を把握する。残高不足の注文は発注前に弾く
- 取引所間の資金リバランスは当面**手動**(少額のうちは頻度が低い)。日次サマリメールに各社残高を載せて判断材料にする

### 8.6 実装上の注意
- ccxt の対応状況: bitFlyer / bitbank は対応、**GMOコイン・Binance Japan は非対応または薄い**ため、この2社は REST もネイティブアダプタを書く前提で工数を見積もる
- 運用ルール「便りがないのは悪い便り」: 日次サマリメールが**届かないこと自体を異常**とみなす(プロセスごと死ぬとアラートも出ないため)
